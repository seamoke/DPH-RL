...
class AgentLoopWorker:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        if 'tools_kwargs' in batch.non_tensor_batch:
            tools_kwargs = batch.non_tensor_batch["tools_kwargs"].repeat(n, axis=0)
            if 'sql_exec_result' in tools_kwargs[0]:
                tools_kwargs = [kwarg['sql_exec_result']['create_kwargs']['ground_truth'] for kwarg in tools_kwargs]
            else:
                tools_kwargs = [None for kwarg in tools_kwargs]
        else:
            tools_kwargs = [None for _ in range(len(agent_names))]

        for agent_name, messages, tools_kwarg in zip(agent_names, raw_prompts, tools_kwargs):
            tasks.append(asyncio.create_task(self._run_agent_loop(agent_name, messages.tolist(), sampling_params,tools_kwarg)))
    ...
    async def _run_agent_loop(
        self, agent_name: str, messages: List[Dict[str, Any]], sampling_params: Dict[str, Any], tools_kwarg: Dict = None
    ) -> AgentLoopOutput:
        ...
        output = await agent_loop.run(messages, sampling_params, tools_kwarg)
        return output
